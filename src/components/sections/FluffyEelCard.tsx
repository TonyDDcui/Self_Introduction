...
type MenuItem = { fullName: string; url: string; pushedAt: string };
type ActiveRepo = { owner: string; name: string; fullName: string; url: string };

export default function FluffyEelCard() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [menu, setMenu] = useState<MenuItem[]>([]);
  const [activeRepo, setActiveRepo] = useState<ActiveRepo>({
    owner: "TonyDDcui",
  ...
  const load = (repoFullName?: string) => {
    ...
    fetch(`/api/github/repo-card${qs}`, { signal: ac.signal, cache: "no-store" })
      .then(async (res) => {
        ...
        setMenu(Array.isArray(json.menu) ? json.menu : []);
        setActiveRepo(json.activeRepo);
            ...
            <div className="header">
              <div className="top-header">
                <div className={styles.menuWrap} ref={menuRef}>
                  ...
                  {menuOpen ? (
                    <div className={styles.menu} role="menu" aria-label="选择仓库">
                      {menu.length === 0 ? (
                        <div className={styles.menuItemMuted}>暂无仓库</div>
                      ) : (
                        menu.map((m) => (
                          <button
                            key={m.fullName}
                            ...
                            data-active={m.fullName === activeRepo.fullName ? "true" : "false"}
                            onClick={() => {
                              setMenuOpen(false);
                              load(m.fullName);
                            }}
                          >
                            <span className={styles.menuRepo}>{m.fullName}</span>
                          </button>
                        ))
                      )}
                    </div>
                ...
                <div className="repo">
                  <span className="repo-owner">{activeRepo.owner}</span>
                  <span className="repo-slash">/</span>
                  <span className="repo-name">{activeRepo.name}</span>
                </div>
                ...
                {loading && items.length === 0 ? (
                  ...
                  <div className="pr">
                    <div className="pr-text">
                      ...
                      <div className="pr-desc">该仓库暂无 commit</div>
                    </div>
                  </div>
                ) : (
                  items.map((it) => (
